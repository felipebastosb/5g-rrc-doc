...
Como pode ser visto na especificação de mensagem abaixo, a camada RRC apenas define o tipo de criptografia e o equipamento do usuário responde se consegiu implementar ou não o sistema de segurança.
...
Quando por algum motivo o enlace de radio é quebrado, o usuário pode enviar a rede um pedido de reconexão mantendo todas os parâmetros que já haviam sido definidos inicialmente, inclusive a criptografia. Casa a estação rádio base ainda tenha as infromações desse usuário, uma mensagem de reestabelecimento de conexão é enviada contendo todas as informações da conexão anterior, caso contrario a rede inicia os procedimentos para uma nova conexão.
